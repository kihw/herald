...
import (
	...
	"github.com/herald-lol/herald/backend/internal/repository"
...
func NewAnalyticsService(db *sql.DB, redisClient interface{}) *AnalyticsService {
...
// NewAnalyticsServiceFull creates a new analytics service with all dependencies
func NewAnalyticsServiceFull(db *sql.DB, matchRepo *repository.MatchRepository, playerRepo *repository.PlayerRepository, redisService *RedisService) *AnalyticsService {
	return &AnalyticsService{
		db:           db,
		matchRepo:    matchRepo,
		playerRepo:   playerRepo,
